...
enum ZerocoinSpendStatus {
    ...
    ZSmrtc_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZSmrtc_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZSmrtcOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    CzSmrtcWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzSmrtcTracker> zpivTracker;
    ...
    void setZWallet(CzSmrtcWallet* zwallet)
    {
        zwalletMain = zwallet;
        zpivTracker = std::unique_ptr<CzSmrtcTracker>(new CzSmrtcTracker(strWalletFile));
    }

    CzSmrtcWallet* getZWallet() { return zwalletMain; }
    ...
    /** zSmrtc reset */
    boost::signals2::signal<void()> NotifyzSmrtcReset;
