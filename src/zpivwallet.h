...
#ifndef SMRTC_ZSmrtcWALLET_H
#define SMRTC_ZSmrtcWALLET_H
...
public:
    CzSmrtcWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZSmrtc(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZSmrtc(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //SMRTC_ZSmrtcWALLET_H
