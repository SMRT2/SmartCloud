...
CzSmrtcWallet::CzSmrtcWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZSmrtcSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZSmrtcSeed_deprecated()) {
                    LogPrintf("%s: Updated zSmrtc seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzSmrtcWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZSmrtcCount(nCountLastUsed);
    else if (!walletdb.ReadZSmrtcCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzSmrtcWallet::Lock()
{
    seedMaster = 0;
}

void CzSmrtcWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzSmrtcWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZSmrtc(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzSmrtcWallet::LoadMintPoolFromDB()
{
...
void CzSmrtcWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzSmrtcWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzSmrtcWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzSmrtcWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZSmrtc(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzSmrtcWallet::SeedToZSmrtc(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzSmrtcWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzSmrtcWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZSmrtcCount(nCountLastUsed);
...
void CzSmrtcWallet::GenerateDeterministicZSmrtc(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzSmrtcWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzSmrtcWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
