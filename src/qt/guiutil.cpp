...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a SMRTC address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no SMRTC: URI
...
{
    // Convert smrtc:// to smrtc:
    //
    //    Cannot handle this later, because smrtc:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open smrtc.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "SMRTC.lnk";
}
...
{
    return GetAutostartDir() / "smrtc.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a smrtc.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=SMRTC\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the smrtc app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add smrtc app to startup item list
