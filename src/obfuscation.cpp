    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat SMRTC and make it unusable. To
...
bool CObfuscationPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeSmrtcAmount * COIN - pwalletMain->GetAnonymizedBalance();
...
void ThreadCheckObfuScationPool()
{
    ...
    RenameThread("smrtc-obfuscation");
