...
// Copyright (c) 2016-2018 The SMRTC developers
...
public:
    ...
    bool WriteZSmrtcSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZSmrtcSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZSmrtcSeed_deprecated(uint256& seed);
    bool EraseZSmrtcSeed();
    bool EraseZSmrtcSeed_deprecated();

    bool WriteZSmrtcCount(const uint32_t& nCount);
    bool ReadZSmrtcCount(uint32_t& nCount);
